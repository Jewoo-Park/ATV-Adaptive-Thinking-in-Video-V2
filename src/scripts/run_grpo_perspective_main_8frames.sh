...
if [[ "${VIRTUAL_ENV:-}" == *".venv_grpo"* ]]; then
  echo "[MAIN-PERSPECTIVE-8F] Reusing active venv_grpo: ${VIRTUAL_ENV}"
else
...
export OUTPUT_DIR="${OUTPUT_DIR:-${_GRPO_RUN_ROOT}/grpo_perspective_sftbase_strategy_fix_main_8frames}"
export RUN_NAME="${RUN_NAME:-grpo_perspective_sftbase_strategy_fix_main_8frames_outputs_0626}"
...
echo "[MAIN-PERSPECTIVE-8F] MAX_STEPS=${MAX_STEPS}, NUM_TRAIN_EPOCHS=${NUM_TRAIN_EPOCHS}, LEARNING_RATE=${LEARNING_RATE}"
echo "[MAIN-PERSPECTIVE-8F] PER_DEVICE_TRAIN_BATCH_SIZE=${PER_DEVICE_TRAIN_BATCH_SIZE}, GRADIENT_ACCUMULATION_STEPS=${GRADIENT_ACCUMULATION_STEPS}"
