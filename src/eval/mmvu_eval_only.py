...
def main() -> None:
    ...
    print(f"[mmvu_eval] samples={len(rows)} test_file={test_path}", flush=True)
    ...
    if args.backend == "vllm" and not _HAS_VLLM:
        raise SystemExit("[mmvu_eval] --backend=vllm but vLLM is not importable in this environment.")
    ...
    if use_vllm:
        ...
        print("[mmvu_eval] backend=vllm (loaded); running inference …", flush=True)
        sp = SamplingParams(temperature=args.temperature, max_tokens=args.max_completion_length, n=1)
    else:
        print("[mmvu_eval] backend=hf (vLLM unavailable or disabled); running inference …", flush=True)
        import torch
